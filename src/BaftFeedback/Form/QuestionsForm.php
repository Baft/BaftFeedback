...
class QuestionsForm extends Form implements ServiceLocatorAwareInterface, 
        FeedbackFormInterface
{
...
//         $this->setHydrator(new SubmissionDataHydrator($this->getNamespace()));
     ...
     *
     * @param
     *            BaftfeedbackFeedback | BaftfeedbackFeedbackSubject
     *            $feedback_subject
     * @throws \Exception
     */
    protected function buildFieldset ()
    {
        if (! isset($this->feedback))
            throw new \Exception(
                    __METHOD__ .
                             " feedback requird for generating questions form");
        
        $feedbackEntity = $this->getFeedback();
        
        $feedbackService = $this->getServiceLocator()->get(
                'BaftFeedback\Service\feedback');
        ...
        $this->getServiceLocator()
            ->get('FormElementManager')
            ->setService($questionsNamespace, $feedbackFieldset);
        
        $formFactory = $feedbackFieldset->getFormFactory();
        $formFactory->setFormElementManager(
                $this->getServiceLocator()
                    ->get('FormElementManager'));
        // ############################ create questions group
        ...
        if (! empty($questionGroups))
            foreach ($questionGroups as $group) {
                ...
                $feedbackFieldset->add($groupFieldset, 
                        [
                                'priority' => $groupFieldset->getGroup()
                                    ->getOrder()
                        ]);
            }
    ...
    /**
     * populate submission data to form
     * @param BaftfeedbackFeedbackSubmission $submission
     */
    public function populateSubmission(BaftfeedbackFeedbackSubmission $submission){
            $submissionService=$this->getServiceLocator()->get ( 'BaftFeedback\Service\submission' );
            $submissionLastData = $submissionService->getSubmissionLastData ( $data  );
            $formDataObject = (new SubmissionDataHydrator ( $this->getNamespace () ))->hydrate ( $submissionLastData, $this->getObject () );
            return parent::populateValues($formDataObject,true);
    }
    
    /**
     * make one level array from multi level form data
     * keep fieldset name as namespace for elements name :
     * [fieldsetName].../[filedName]
     * [ nameSpace ] => nodeValue
     *
     * @param array $data            
    ...
    {
        return $this->get($this->getFeedback()
            ->getQuestionsNamespace());
    }
    ...
    public function getNamespace ()
    {
        if (! isset($this->questionsNamespace)) {
            $this->setNamespace($this->getFeedback()
                ->getQuestionsNamespace());
        }
                                ...
                                new \Zend\Validator\Callback(
                                        [
                                                        ...
                                                        $context = array()) use ( $form)
                                                {
                                                    ...
                                                    return $form->getServiceLocator()
                                                        ->get(
                                                            'BaftFeedback\Service\submission')
                                                        ->appurtenantSubmission(
                                                            $value, 
                                                            $context['feedback']);
                                                },
                                                'messages' => [
