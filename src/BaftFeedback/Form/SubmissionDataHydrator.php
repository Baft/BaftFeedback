...
use Zend\Json\Json;
/**
 * extact : transform object data (from form) to rowSet format data (for db)
 * hydrate : transform database rowSet format to form object 
 * @author cmd
 *
 */
class SubmissionDataHydrator implements \Zend\Hydrator\HydratorInterface{
	
	private $namespace;

	public function __construct( $namespace = null) {
		
		if($namespace instanceof \BaftFeedback\Form\FeedbackFormInterface){
			$namespace=$namespace->getNamespace();
		}
		
		if( is_null ( $namespace ))
			throw new \Exception("submission hydrator need feedback form namespace");
	...
	public function extract($object) {
		...
		if (! isset ( $formBindObject [$namespace] ))
			throw new \Exception ( __METHOD__ . " : feedback form namespace \"" . $namespace . "\" dose not exist in form object " );
